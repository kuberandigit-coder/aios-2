...
raw = json.load(open(p("2026-07-07_mahima_req1_product_performance_raw.json"), encoding="utf-8"))
...
for r in raw:
    ...
    rows.append({
        ...
        "sku": r["internal_sku"] or "", "imp": r["impressions"] or 0, "clicks": clicks,
...
for r in rows:
    rows_data.append({
        "c": r["cname"], "i": r["item"], "sk": r["sku"], "imp": r["imp"], "cl": r["clicks"],
...
table.rt{width:100%;border-collapse:collapse;font-size:12.5px;min-width:1000px;}
...
for(const r of ROWS){ r._i=(r.i||'').toLowerCase(); r._sk=(r.sk||'').toLowerCase(); r._c=(r.c||'').toLowerCase(); }
...
function rowHtml(r){
  return `<tr>
    ...
    <td class="wrap-cell">${esc(r.sk)||'<span class="na">no SKU match</span>'} <span class="na">(${esc(r.i)})</span></td>
...
function applyFilter(){
  ...
  filtered=ROWS.filter(r=>{
    const textHit=!s||r._i.includes(s)||r._sk.includes(s)||r._c.includes(s);
...
<header class="top">
  ...
  <div class="chips">
    <span class="chip">{total:,} product rows &middot; 4 of 5 active campaigns (see limitations)</span>
...
<div class="cards">
  ...
  <div class="card"><div class="l">Total Profit After Ads</div><div class="v na">Data Missing</div></div>
...
<div class="toolbar">
  <input id="q" type="text" placeholder="Search product ID, SKU, or campaign&hellip;">
...
<table class="rt">
<thead><tr>
  <th>Campaign</th><th>Product (SKU / Item ID)</th><th>Impressions</th><th>Clicks</th><th>CTR</th>
  <th>Avg CPC</th><th>Cost</th><th>Conversions</th><th>Conv. Rate</th><th>Conv. Value</th><th>ROAS</th><th>Suggested Action</th>
</tr></thead>
...
<div class="legend">
  ...
  &mdash; pre-computed in the source table (<code>mahima_action</code> column). The exact rule logic behind these values has <strong>not yet been independently verified</strong> against the Scale/Maintain/Optimize/Pause criteria in this requirement &mdash; treat as provisional pending confirmation.
</div>

<div class="sources">
  <strong>Data Sources &amp; Calculation Rules</strong><br>
  <strong>Source:</strong> PostgreSQL <code>staging_ai.cppc_workbook_product_performance_v1</code>, filtered to campaign IDs 20763699505, 23684789991, 23053104908, 23431543574, 23926509987 (Mahima's ledsone.de campaigns, confirmed via <code>staging_ai.cppc_campaign_truth_registry_v1</code>, owner = "Mahi"). Read-only SQL only &mdash; no data modified.<br>
  <strong>CTR</strong> = Clicks &divide; Impressions &middot; <strong>Avg CPC</strong> = Cost &divide; Clicks &middot; <strong>Conv. Rate</strong> = Conversions &divide; Clicks &middot; <strong>ROAS</strong> = Conv. Value &divide; Cost. All computed directly from the source row&#39;s spend/clicks/impressions/conversions/revenue fields.<br>
  <strong>Suggested Action:</strong> taken as-is from the source table&#39;s <code>mahima_action</code> column (pre-computed elsewhere; rule not yet reverse-engineered or confirmed with Mahima).
</div>
...
<div class="limits">
  ...
  2. <strong>Product Price and Product Cost are Data Missing for every row.</strong> No table anywhere in the database has a working join from this dataset&#39;s product IDs to Shopify/Merchant Center pricing, and no product-cost/COGS source has real populated values (checked <code>development.sku_cogs</code> &mdash; empty; <code>staging_ai.cppc_cogs_truth_model_v1</code> &mdash; all cost fields NULL). Gross Profit and Profit After Ads therefore cannot be calculated and are omitted from this version.<br>
  3. <strong>Feed Status and Missing Attribute are Data Missing.</strong> The correct source table (<code>raw_data.gmc_product_diagnostics_daily</code>) is completely empty (0 rows) &mdash; no Feed Status/Missing Attribute data exists anywhere in the database.<br>
  4. <strong>Last 7 Days ROAS / Last 30 Days ROAS are not shown separately.</strong> The source table stores one aggregate window per row with no day-level history to compute a rolling comparison from.<br>
  5. <strong>Product title is not available</strong> &mdash; shown as internal SKU (where matched, 94 of 680 rows) or the raw Google product/item ID otherwise.<br>
...
<div class="foot">
  <strong>Scope:</strong> Mahima Requirement 1 &mdash; Product Performance Report, ledsone.de Google Ads, product-level view across Mahima&#39;s active campaigns (see Data Sources above for exact IDs).<br>
  <strong>PASS/FAIL note:</strong> per this task&#39;s own rule, a fully complete PASS requires Product Cost, Feed Status, and Missing Attribute to be present &mdash; those remain Data Missing here by design (not invented), so this build should be read as a <strong>partial, clearly-labeled interim version</strong> pending the data-gap decisions documented in the AIOS evidence/handover files.
</div>
...
<script>{js}</script>
    ...
    overall_roas=overall_roas, scale_n=scale_n, pause_n=pause_n,
...
out2 = "C:/Users/PC/OneDrive/Desktop/kuberan web/reports/mahima/mahima-requirement-1-product-performance-report.html"
open(out1, "w", encoding="utf-8").write(page_html)
open(out2, "w", encoding="utf-8").write(page_html)
print("written:", out1, len(page_html)//1024, "KB")
print("written:", out2, len(page_html)//1024, "KB")
print("total rows:", total, "total_cost:", total_cost, "total_value:", total_value, "overall_roas:", overall_roas)
