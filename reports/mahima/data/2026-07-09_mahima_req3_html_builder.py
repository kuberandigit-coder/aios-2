...
data = json.load(io.open(SP + r"\req3_rows.json", encoding="utf-8"))
...
for r in rows:
    compact.append({
        ...
        "va": r["conv_value"],
        "ro": r["roas"],
        "cpco": r["cost_per_conv"],
        "qi": r["query_intent"],
        "nk": r["existing_negative_kw"],
        "ro7": r["roas_7d"],
        "ro30": r["roas_30d"],
        "tr": r["trend"],
        "pr": r["priority"],
...
campaign_counts = Counter(r["campaign"] for r in rows)
campaign_options = "".join(
    f'<option value="{html.escape(c)}">{html.escape(c)} ({n_})</option>'
    for c, n_ in sorted(campaign_counts.items(), key=lambda x: -x[1])
)

action_counts = Counter(r["action"] for r in rows)
action_options = "".join(
    f'<option value="{html.escape(a)}">{html.escape(a)} ({n_})</option>'
    for a, n_ in sorted(action_counts.items(), key=lambda x: -x[1])
)

intent_counts = Counter(r["query_intent"] for r in rows)
intent_options = "".join(
    f'<option value="{html.escape(a)}">{html.escape(a)} ({n_})</option>'
    for a, n_ in sorted(intent_counts.items(), key=lambda x: -x[1])
)

priority_counts = Counter(r["priority"] for r in rows)
priority_options = "".join(
    f'<option value="{html.escape(a)}">{html.escape(a)} ({n_})</option>'
    for a, n_ in sorted(priority_counts.items(), key=lambda x: -x[1])
)
...
<header class="top">
  ...
  <div class="sub">Which search terms should Mahima keep, exclude, or scale based on conversions, ROAS, query intent, and wasted spend? &middot; Account: <strong>ledsone.de</strong> (Google Ads account_id 9031058245) &middot; Date range: <strong>last 30 days as of 2026-07-09</strong></div>
  <div class="chips">
    <span class="chip">{summary['total_terms']:,} search terms (clicks &gt; 0 in last 30 days)</span>
    ...
    <span class="chip warn">No negative keywords currently configured on this account (0 of 128 keywords are negative)</span>
  </div>
</header>
...
<div class="legend">
  ...
  <strong>Priority:</strong> <span class="p-high">High</span> / <span class="p-medium">Medium</span> / <span class="p-low">Low</span> &mdash; Exclude rows with &euro;5+ wasted cost = High priority to cut; &euro;0&ndash;5 = Medium; &euro;0 cost = Low. Keep rows with 30-Day ROAS &ge; 2x = High priority to protect/scale; below that = Medium.
</div>
...
<div class="sources">
  ...
  <strong>Date range:</strong> 30-day window = last 30 days from 2026-07-09 (2026-06-10 &rarr; 2026-07-09). 7-day window = last 7 days (2026-07-03 &rarr; 2026-07-09). Rows are search_term &times; campaign &times; match_type, restricted to rows with clicks &gt; 0 in the 30-day window (21,282 distinct terms existed with impressions only; filtering to clicked terms keeps the report focused on spend/keep-cut decisions).<br>
  <strong>CTR</strong> = clicks / impressions. <strong>Avg CPC</strong> = cost / clicks. <strong>Conv. Rate</strong> = conversions / clicks. <strong>ROAS</strong> = conv. value / cost. <strong>Cost/Conv</strong> = cost / conversions. <strong>7-Day / 30-Day ROAS</strong> = conv. value / cost for that window. All divide-by-zero cases render as <span class="na">N/A</span> or 0 as specified &mdash; never fabricated.<br>
...
<div class="limits">
  ...
  3. Report is restricted to the {summary['total_terms']:,} search terms that received at least 1 click in the last 30 days (out of 21,282 distinct terms with any impression) to keep the table focused on spend-relevant decisions.<br>
...
<div class="foot">
  <strong>Requirement:</strong> Mahima Requirement 3 &mdash; Search Terms Report (Keep / Cut / Scale) &middot; <strong>Owner:</strong> Mahima &middot; <strong>Built by:</strong> Claude Code (AIOS) &middot; <strong>Date:</strong> 2026-07-09 &middot; <strong>Status:</strong> Draft for review, read-only PostgreSQL data, no Google Ads changes made.
</div>
