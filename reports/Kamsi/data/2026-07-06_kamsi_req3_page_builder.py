...
<style>
  ...
  *{box-sizing:border-box; margin:0; padding:0;}
  ...
  .fbar{padding:14px 20px; border-bottom:1px solid var(--line); display:flex; gap:14px; flex-wrap:wrap; align-items:center; font-size:12px;}
  .fbar label{display:flex; align-items:center; gap:6px; font-weight:600; color:#42506a;}
  .fbar select{padding:6px 10px; border:1px solid var(--line); border-radius:8px; font-size:12.5px; background:#fff; color:var(--ink); cursor:pointer;}
...
<div class="wrap">
  ...
  <div class="tablebox">
    <div class="fbar">
      <label>Page Type <select id="f_type" onchange="applyFilters()">
        <option value="">All</option><option value="C">Collection</option><option value="B">Blog</option><option value="O">Other</option></select></label>
      <label>Collection <select id="f_coll" onchange="applyFilters()">
        <option value="">All collections</option></select></label>
    </div>
...
function pathType(lp){
  if(lp.indexOf('/collections/')===0) return 'C';
  if(lp.indexOf('/blogs/')===0 || lp.indexOf('/blog/')===0) return 'B';
  return 'O';
}
function collectionOf(lp){
  if(lp.indexOf('/collections/')!==0) return null;
  return lp.slice('/collections/'.length).split('/')[0] || null;
}
let curD = '30';
function populateCollections(rows){
  const sel = document.getElementById('f_coll'), cur = sel.value;
  const colls = Array.from(new Set(rows.filter(r => pathType(r[0])==='C').map(r => collectionOf(r[0])))).sort();
  sel.innerHTML = '<option value="">All collections</option>' + colls.map(c => '<option value="' + esc(c) + '">' + esc(c) + '</option>').join('');
  if(colls.includes(cur)) sel.value = cur;
}
function filteredRows(rows){
  const ft = document.getElementById('f_type').value, fc = document.getElementById('f_coll').value;
  return rows.filter(r => {
    if(ft && pathType(r[0]) !== ft) return false;
    if(fc && collectionOf(r[0]) !== fc) return false;
    return true;
  });
}
function applyFilters(){ render(curD); }
function render(d){
  curD = d;
  ...
  populateCollections(w.r);
  const rows = filteredRows(w.r);
  document.getElementById('tbar').textContent = 'Showing ' + rows.length + ' of top ' + w.r.length + ' landing pages by sessions (of ' + w.n.toLocaleString() +
