# Kamsi Req 2 — GSC daily (per-page, per-day) performance, 2026-01-01 to 2026-06-30
import json
...
def fetch(dimensions, contains):
    ...
    while True:
        ...
        rows = r.get("rows", [])
        rows_all += rows
        print("  fetched", len(rows_all), "so far for", contains)
        if len(rows) < 25000:
            break
        start += 25000
    return rows_all
...
for r in daily_rows:
    ...
    if key in seen:
        continue
    seen.add(key)
    clean.append({"page": u, "date": d, "clicks": int(r["clicks"]), "impressions": int(r["impressions"]),
...
out = r"C:\Users\PC\OneDrive\Desktop\kuberan web\reports\Kamsi\data\2026-07-08_kamsi_req2_gsc_6month_daily_raw.json"
