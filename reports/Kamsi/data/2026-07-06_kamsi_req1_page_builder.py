...
# 2026-07-06 SOURCE MIGRATION: all three metric fields now Shopify-only (Kamsi Req 1 update).
# Merges: FULL Shopify catalog + inventory (bulk export JSONL, all products+variants+inventoryQuantity)
#         + Shopify orders last 90d (bulk export, cancelled excluded) -> units sold 90d + last order date
#         PostgreSQL is NOT used for any final field.
...
GEN = "2026-07-06"
WINDOW = "2026-04-07 → 2026-07-06 (90 days)"

def p(*a): return os.path.abspath(os.path.join(*a))

orders = {}
for r in csv.DictReader(open(p(DATA, "2026-07-06_kamsi_req1_shopify_orders_90d.csv"), encoding="utf-8")):
    if r["sku"]: orders[r["sku"]] = (int(r["units_sold_90d"]), r["last_order_date"])

stock = {}
for r in csv.DictReader(open(p(DATA, "2026-07-06_kamsi_req1_shopify_stock_by_sku.csv"), encoding="utf-8")):
...
n_draft = 0
with open(p(DATA, "2026-07-06_kamsi_req1_shopify_catalog_inventory.jsonl"), encoding="utf-8") as f:
    for line in f:
        o = json.loads(line)
...
<div class="wrap">
  ...
  <header>
    ...
    <div class="filters">
      ...
      <span class="chip">Sources: Shopify only — catalog · orders · inventory (Admin API)</span>
      <span class="chip">Units Sold (90d), Current Stock, and Last Order Date are sourced from Shopify.</span>
    </div>
  </header>
...
{cat_opts}
  ...
  <div class="footnotes">
    ...
    <strong>Data source note:</strong> Units Sold (90d), Current Stock, and Last Order Date are sourced from Shopify (Admin API bulk exports, read-only, {GEN}). PostgreSQL is no longer used for these fields (source migration approved in Kamsi Req 1 update, 2026-07-06).<br>
    <strong>Units Sold (90d) &amp; Last Order Date:</strong> Shopify order line items from all orders created in {WINDOW}, cancelled orders excluded (order.cancelledAt), quantity summed per SKU; Last Order Date = latest non-cancelled order date per SKU. SKUs with no Shopify order in the window show 0 and "—".<br>
    <strong>Current Stock:</strong> Shopify sellable/available inventory per SKU (ProductVariant.inventoryQuantity — available quantity across all Shopify locations), snapshot {GEN}. PostgreSQL warehouse stock is NOT used. {fmt(n_nostock)} SKUs without a Shopify inventory record show "—" and are never classified Slow-Moving (stock unknown ≠ stock &gt; 100).<br>
    ...
    <strong>Data files:</strong> <code>reports/Kamsi/data/2026-07-06_kamsi_req1_*</code> · Builder: <code>2026-07-06_kamsi_req1_page_builder.py</code> (rerun to regenerate).
  </div>
