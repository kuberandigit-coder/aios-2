#!/usr/bin/env node
// Bulk regenerator for Sonya's Jan-Jul snapshots (same pattern as
// bulk-sajeepan-refresh.js) -- strictly sequential with cooldowns so
// retries don't stack up as concurrent Shopify scans.
const fs = require('fs');
...
async function fetchMonth(month) {
  ...
  const url = `${BASE_URL}/api/sales?staff=sonya-ads&month=${month}&refresh=1`;
  ...
  const outPath = path.join(DATA_DIR, `sonya-uk-ads-sales-${month}.json`);
