#!/usr/bin/env node
// Bulk regenerator for sales25.js group snapshots, one month at a time.
// Usage: node scripts/bulk-sales25-refresh.js <month> [group1,group2,...]
const fs = require('fs');
...
async function fetchGroup(group) {
  const url = `${BASE_URL}/api/sales25?group=${group}&month=${month}&refresh=1`;
  ...
  const outPath = path.join(DATA_DIR, `sales25-${group}-${month}.json`);
