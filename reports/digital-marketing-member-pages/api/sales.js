...
async function handleOrganic(req, res, monthConfig, forceRefresh, startTime) {
  ...
  if (staff === 'theekshy-ads') {
    // Theekshy Ads tab — added 2026-07-22, ledsone.co.uk (STORE_DOMAIN_UK),
    // UK Google Ads team. Confirmed rule: an order belongs to Theekshy if
    // its first-session utm_term exactly matches "theekshy" (case-
    // insensitive). She joined in March 2026, so only March onward has
    // her data — January/February are not applicable. Store-wide, NOT
    // product-scoped, all months including live July.
    ...
    const theekshyPayload = {
      success: true,
      staff: { name: 'Theekshy', department: 'Google Ads (Paid Search)', store: 'ledsone.co.uk' },
      reportPeriod: { month: monthConfig.month, label: monthConfig.label, start: monthConfig.startISO, endExclusive: monthConfig.endISO, timezone: 'Europe/London' },
      supportedMonths: SUPPORTED_MONTHS,
      isLive: monthConfig.isLive,
      source: {
        scope: `store-wide (NOT product-scoped) — an order belongs to Theekshy if its first-session utm_term exactly matches "theekshy" (case-insensitive). Joined March 2026 (confirmed rule, 2026-07-22)`,
        orders: 'Shopify Admin GraphQL API',
        journey: 'Shopify customerJourneySummary',
      },
      campaignList: [...byCampaign.keys()].sort(),
