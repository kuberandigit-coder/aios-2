...
const GROUPS = [
  ...
  },
  {
    key: 'mahima',
    name: 'Mahima',
    department: 'Google Ads (Paid Search) + Organic (product-scoped)',
    scope: 'ported from api/sales.js staff=mahima-ads (first-session source/medium/campaign/term/content contains "mahi", or campaign contains "lowclick") OR the order contains any of Mahima\'s owned product IDs (the same product-scoping already used to split her Organic sales from Sukirtha\'s on the main dashboard).',
    match: (utm, fv, journey, month, order) => isMahimaAdsMatch(utm) || orderHasMahimaProduct(order),
    matchValue: (utm, fv, journey, month, order) => utm.campaign || (orderHasMahimaProduct(order) ? '(owned product)' : '(no campaign value)'),
  },
  {
    key: 'sukirtha',
    name: 'Sukirtha',
