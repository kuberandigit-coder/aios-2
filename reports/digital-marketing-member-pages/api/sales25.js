...
const GROUPS = [
  ...
  },
  {
    ...
    scope: 'the order matches Organic\'s rule (see below) AND contains one of Kamsi\'s owned product IDs, EXCLUDING any Direct order whose 2nd session is Google Ads paid search (those go to Sajeepan/Sonya instead, checked earlier — per user request 2026-07-30, "pure organic" Direct only). Checked before Organic and Dilaksi.',
  ...
  },
  {
    ...
    name: 'Dilaksi',
    department: 'Organic (product-scoped)',
    scope: 'the order matches Organic\'s rule (see below) AND contains one of Dilaksi\'s owned product IDs, EXCLUDING any Direct order whose 2nd session is Google Ads paid search (those go to Sajeepan/Sonya instead — per user request 2026-07-30). Checked after Kamsi (1 product ID appears in both lists as given by the user, resolves to Kamsi) and before Organic.',
...
// Direct-traffic reclassification (added 2026-07-30, per user request, same
// as salesuk.js): a "Direct" first session can still have a 2nd session
// that DOES carry an ad signal — Shopify's own per-session classifier
// (classifications[1].classification) is used rather than re-parsing UTMs.
...
async function handleGroup(req, res, monthConfig, forceRefresh, groupDef) {
  ...
  for (const order of orders) {
    ...
    row.matchedCampaign = (ov && ov.source === 'sales25')
