...
## UPDATE (2026-09-19) — Live end-to-end confirmed, plus post-launch fixes and one new feature (TASK COMPLETE)

### Live confirmation across all 5 steps
Since the last update, the user actively used the deployed feature against real production data across
multiple sessions: ran real Step 01 refreshes, ran real Step 02 scans (catching and reporting the
`product_type` fan-out bug, fixed same-day), ran real Step 03 density calculations, ran real Step 04
generation and used the Approve/Reject workflow on real suggestions (confirmed via live query: 3 real
`Approved` rows exist), and exercised Step 05's handoff/status/assignment UI. This satisfies the task's
own closure rule -- see the new closure record.

### Bug fixed: unpublished/out-of-stock products leaking into Step 02/04
User reported products with no real availability were still being suggested as source/target pages.
Live-verified against real Shopify data (`totalInventory`/`tracksInventory` fields) that some real products
had `totalInventory: 0`. Fixed: `content_fetch.py` now captures real stock/publish signals; a new
`_is_eligible_page()` in `link_opportunities.py` excludes any Product that isn't `ACTIVE` or is confirmed
out of stock from being used as EITHER a source or a suggested target -- applied once at the shared fetch
point so Step 04 inherits it automatically. Collections/Blogs have no equivalent signal yet -- documented
limitation, not silently assumed safe. (Commit `d627323`.)

### New feature (explicitly requested, scoped to Blog only): HTML preview + copy-to-Shopify editor
User asked for "another level of automation" -- show the real Shopify HTML for a Blog suggestion, preview
the approved link inserted, and let the user copy the result to paste into Shopify themselves (still fully
manual, no Shopify write). Built and iterated through several real user-driven UX fixes:
- `html_preview.py` -- safe anchor insertion via tag/text-segment splitting so a link is never nested
  inside an existing link; live-verified it correctly REFUSED to insert where the anchor phrase only
  existed inside a different existing link, rather than corrupting the page.
- Moved from a cramped inline preview to a dedicated wide (1180px) "Blog HTML Editor" modal with a real
  Before/After side-by-side diff (pure common-prefix/suffix diff, verified in Node against real generated
  HTML), a separate "Edit" button per Blog row, and a pretty-printed/indented HTML code view (verified
  against a real 28KB blog post).
- Fixed real clarity bugs the user found: images with original-page float/width styling broke the preview
  layout (now stripped from the rendered preview only, real HTML untouched); the code view was one
  unreadable wall of text (now indented); "Locate Change" was scrolling the wrong pane through two rounds
  of correction, settled on scrolling the "After" preview pane specifically, per explicit instruction.
- **Real gap found and fixed via user report:** a suggestion could pass Step 02's existing-link check (page
  doesn't link to THIS specific target) yet still be impossible to safely auto-insert, because the anchor
  phrase already exists on the page inside a DIFFERENT existing link. User asked "if not available then
  why showing?" -- valid question. Fixed by pre-computing `html_insertable` at Step 04 generation time
  (reusing the same stored HTML, no live fetch) and surfacing it as a visible column/filter in the table
  and inside the detail popup, instead of only being discoverable after opening the editor. Live-verified:
  55 of 732 real Blog suggestions are genuinely not insertable.

### Separate, unrelated work also completed this session
- Full 151/153-relation Postgres unused-table audit (13 confirmed unused, documented, draft DROP script
  provided, nothing executed) -- see `backend/scripts/drop_unused_tables_2026-09-18.sql`.
- `docs/DATABASE-TABLE-INVENTORY.md` (+ a converted `.docx` version) -- a complete table-by-table inventory
  of the whole dm-dashboard database (usage location + what it stores per table), generated by querying
  `information_schema` and grepping the actual codebase, not guessed.
- One real production incident diagnosed and fixed: a merged Piranav feature (`gsc_live_sync.py`) imported
  `apscheduler`, which was never added to `requirements.txt`, crashing the ENTIRE backend on every route
  (502 everywhere). Root-caused live, fixed by installing the missing dependency on the server.

### No secrets
None recorded in any part of this update.
